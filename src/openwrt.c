...
int openwrt_rap(json_object **ret) {
  ...
  if ((arptable = fopen("/proc/net/arp", "r"))) {
    ...
    while (fgets(line, sizeof(line), arptable) != NULL) {
      remove_newline(line);
      if (sscanf(single_space(line), "%s %s %s %s %s %s %s", ipaddr, hw, flag,
                 macaddr, mask, device, tmp) == 6) {
...
int openwrt_ipv6_neigh(json_object **ret) {
  ...
  if ((ipv6nghtable = popen("ip -6 neigh", "r"))) {
    ...
    while (fgets(line, sizeof(line), ipv6nghtable) != NULL) {
      ...
      if (sscanf(single_space(line), "%s dev %s lladdr %s %s %s", ip6addr,
                 device, macaddr, router, ip6status) == 5 ||
          sscanf(single_space(line), "%s dev %s lladdr %s %s", ip6addr, device,
                 macaddr, ip6status) == 4) {
        t = blobmsg_open_table(&bb, NULL);
...
int openwrt_ipv6_link_local_address(json_object **ret) {
  int rc = SR_ERR_OK;
  FILE *ip_a;
  void *t, *a;
  char line[512];
  ...
  if ((ip_a = popen("ip -6 addr", "r"))) {
    ...
    while (fgets(line, sizeof(line), ip_a) != NULL) {
      remove_newline(line);
      if (sscanf(single_space(line), "%d: %[^:@] %s", &item, device, rest) ==
          3) {
      } else if (sscanf(single_space(line), " inet6 %[^/]/%u scope link",
                        ip6addr, &prefix) == 2) {
        t = blobmsg_open_table(&bb, NULL);
        blobmsg_add_string(&bb, "device", device);
        blobmsg_add_string(&bb, "ip6addr", ip6addr);
