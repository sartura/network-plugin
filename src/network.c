...
static struct value_node *insert_node(struct list_head *list, char *xpath) {
  int rc = SR_ERR_OK;
...
static int insert_sr_node(struct list_head *list, struct json_object *jobj,
                          char *j_name, sr_type_t sr_type, char *xpath) {
  int rc = SR_ERR_OK;
  double res = 0;
  char *end = NULL;
  ...
  CHECK_NULL(item, &rc, cleanup, "failed json_object_object_get_ex for %s",
             j_name);
...
static bool is_l3_member(json_object *i, json_object *d, char *interface,
                         char *device) {
  ...
  for (j = 0; j < N; j++) {
    ...
    if (0 == strcmp(j_name, interface) && strlen(interface) == strlen(j_name)) {
      ...
      if (0 == strcmp(l3_device, device) &&
          strlen(l3_device) == strlen(device)) {
        return true;
      }
    }
  }
...
static struct json_object *get_device_interface(json_object *i, json_object *d,
                                                char *name) {
  struct json_object *res = NULL, *r;
  const char *l3_device = NULL;
  ...
  json_object_object_foreach(d, key, val) {
    if (l3_device && 0 == strcmp(key, l3_device) &&
        strlen(key) == strlen(l3_device)) {
      res = val;
      break;
    }
  }
...
static struct json_object *get_json_interface(json_object *obj, char *name) {
  struct json_object *res = NULL, *r;
...
static int network_operational_operstatus(priv_t *p_data, char *interface_name,
  ...
  char *fmt =
      "/ietf-interfaces:interfaces-state/interface[name='%s']/oper-status";
...
static int network_operational_mac(priv_t *p_data, char *interface_name,
                                   struct list_head *list) {
  int rc = SR_ERR_OK;
  struct json_object *i;
  char *fmt =
      "/ietf-interfaces:interfaces-state/interface[name='%s']/phys-address";
  char xpath[MAX_XPATH];
...
static int network_operational_rx(priv_t *p_data, char *interface_name,
                                  struct list_head *list) {
  int rc = SR_ERR_OK;
  struct json_object *i;
  char *fmt =
      "/ietf-interfaces:interfaces-state/interface[name='%s']/statistics/";
  ...
  CHECK_NULL(i, &rc, cleanup, "failed json_object_object_get_ex for %s",
             "statistics");
...
static int network_operational_tx(priv_t *p_data, char *interface_name,
                                  struct list_head *list) {
  int rc = SR_ERR_OK;
  struct json_object *i;
  char *fmt =
...
static int network_operational_mtu(priv_t *p_data, char *interface_name,
                                   struct list_head *list) {
  int rc = SR_ERR_OK;
  ...
  struct value_node *list_value;
  const char *fmt =
      "/ietf-interfaces:interfaces-state/interface[name='%s']/ietf-ip:ipv4/mtu";
  const char *fmt6 =
      "/ietf-interfaces:interfaces-state/interface[name='%s']/ietf-ip:ipv6/mtu";
  char xpath[MAX_XPATH];
...
static int network_operational_ip(priv_t *p_data, char *interface_name,
                                  struct list_head *list) {
  int rc = SR_ERR_OK;
  const char *ip;
  ...
  for (j = 0; j < N; j++) {
    ...
    const char *fmt = "/ietf-interfaces:interfaces-state/interface[name='%s']/"
                      "ietf-ip:ipv4/address[ip='%s']/prefix-length";
  ...
  for (j = 0; j < N6; j++) {
    ...
    const char *fmt = "/ietf-interfaces:interfaces-state/interface[name='%s']/"
                      "ietf-ip:ipv6/address[ip='%s']/prefix-length";
    sprintf(xpath, fmt, interface_name, ip);
    list_value = calloc(1, sizeof *list_value);
    sr_new_values(1, &list_value->value);
    sr_val_set_xpath(list_value->value, xpath);
...
static int network_operational_neigh6(priv_t *p_data, char *interface_name,
                                      struct list_head *list) {
  int rc = SR_ERR_OK;
  struct json_object *table, *iter_object;
  /* const char *ubus_result; */
  const char *fmt = "/ietf-interfaces:interfaces-state/interface[name='%s']/"
                    "ietf-ip:ipv6/neighbor[ip='%s']/%s";
  char xpath[MAX_XPATH];

  json_object_object_get_ex(p_data->n, "neighbors", &table);
  CHECK_NULL(table, &rc, cleanup, "failed json_object_object_get_ex for %s",
             "neighbors");
...
static int network_operational_neigh(priv_t *p_data, char *interface_name,
                                     struct list_head *list) {
  int rc = SR_ERR_OK;
  struct json_object *table, *iter_object;
  /* const char *ubus_result; */
  const char *fmt = "/ietf-interfaces:interfaces-state/interface[name='%s']/"
                    "ietf-ip:ipv4/neighbor[ip='%s']/link-layer-address";
  char xpath[MAX_XPATH];
...
int operstatus_transform(priv_t *p_data, char *interface_name,
                         struct list_head *list) {
  int rc = SR_ERR_OK;
  ...
  // CHECK_RET(rc, cleanup, "Failed to get ubus state data: %s",
  // sr_strerror(rc));
...
int phy_interfaces_state_cb(priv_t *p_data, char *interface_name,
                            struct list_head *list) {
  int rc = SR_ERR_OK;
  struct json_object *t, *i;
  const char *ubus_result;
  ...
  json_object_object_foreach(p_data->d, key, val) {
    if (0 == strcmp(key, interface_name) &&
        strlen(key) == strlen(interface_name)) {
      i = val;
      break;
    }
  }
  ...
  CHECK_NULL_MSG(list_value, &rc, cleanup, "failed insert_node");
  sr_val_set_xpath(list_value->value, xpath);
  sr_val_set_str_data(list_value->value, SR_IDENTITYREF_T,
...
int sfp_data_cb(struct json_object *p_data, struct list_head *list) {
  int rc = SR_ERR_OK;
  if (!p_data) {
    return rc;
  }
  ...
  const char *fmt_rx =
      "/ietf-interfaces:interfaces-state/interface[name='wan']/"
      "terastream-interfaces-opto:rx-pwr";
  rc = insert_sr_node(list, p_data, "rx-pwr", SR_DECIMAL64_T, (char *)fmt_rx);
  CHECK_RET(rc, cleanup, "failed insert_sr_node: %s", sr_strerror(rc));

  const char *fmt_tx =
      "/ietf-interfaces:interfaces-state/interface[name='wan']/"
      "terastream-interfaces-opto:tx-pwr";
  rc = insert_sr_node(list, p_data, "tx-pwr", SR_DECIMAL64_T, (char *)fmt_tx);
  CHECK_RET(rc, cleanup, "failed insert_sr_node: %s", sr_strerror(rc));

  const char *fmt_current =
      "/ietf-interfaces:interfaces-state/interface[name='wan']/"
      "terastream-interfaces-opto:current";
  rc = insert_sr_node(list, p_data, "current", SR_DECIMAL64_T,
                      (char *)fmt_current);
  CHECK_RET(rc, cleanup, "failed insert_sr_node: %s", sr_strerror(rc));

  const char *fmt_voltage =
      "/ietf-interfaces:interfaces-state/interface[name='wan']/"
      "terastream-interfaces-opto:voltage";
  rc = insert_sr_node(list, p_data, "voltage", SR_DECIMAL64_T,
                      (char *)fmt_voltage);
  CHECK_RET(rc, cleanup, "failed insert_sr_node: %s", sr_strerror(rc));
